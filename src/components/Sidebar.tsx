    ...
    <aside className="fixed left-0 top-0 h-full w-[280px] bg-[#510047] text-white flex flex-col py-6 z-50 shadow-xl border-r border-[#3c0034]">
      ...
      <div className="px-6 mb-10 flex items-center gap-3">
        <div className="w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center border border-white/20">
          <GraduationCap className="w-6 h-6 text-[#76f7e8]" />
        </div>
        <div>
          <h1 className="font-headline-sm text-lg font-bold text-white tracking-tight">EduCorp LMS</h1>
          <p className="text-xs text-white/60 font-medium tracking-wide">Enterprise Learning</p>
        </div>
      </div>
      ...
      <nav className="flex-grow space-y-1">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon className={`w-5 h-5 transition-transform duration-300 group-hover:scale-110 ${isActive ? 'text-[#76f7e8]' : 'text-white/60 group-hover:text-white'}`} />
      ...
      <div className="mt-auto px-6 space-y-4">
        ...
        <div className="p-4 rounded-xl bg-white/5 border border-white/10 text-center">
          <p className="text-[11px] font-semibold tracking-wider text-white/50 uppercase mb-2">Enterprise Plan</p>
          <button 
            onClick={() => alert("Upgrade request sent to accounts department! Standard tier active.")}
        ...
        <div className="pt-4 border-t border-white/10 space-y-1">
          <a 
            href="#" 
            onClick={(e) => { e.preventDefault(); alert("EduCorp Support: How can we assist you today?"); }}
          ...
          <a 
            href="#" 
            onClick={(e) => { e.preventDefault(); if(confirm("Are you sure you want to log out?")) alert("Sign out simulated successfully!"); }}
            className="flex items-center gap-3 py-2 text-white/70 hover:text-white transition-colors"
          >
